...
// Servlet에서는 메소드 단위로 맵핑이 안되고, class 단위로만 맵핑이 된다.  
@WebServlet("/myDispatcherServlet")  // http://localhost/ch2/myDispatcherServlet?year=2021&month=10&day=1
public class MyDispatcherServlet extends HttpServlet { // 서블릿은 HttpServlet을 꼭 상속 받아야함. 
	@Override
		...
		try {
			...
			for(int i=0;i<paramArr.length;i++) {
				...
				if(paramType==Model.class) {
					...
					argArr[i] = request; // request가 있으면 같이 넘겨줌
				} else if(paramType==HttpServletResponse.class) {
					argArr[i] = response; // response가 있으면 같이 넘겨줌
				} else if(value != null) {  // map에 paramName이 있으면,
					// value와 parameter의 타입을 비교해서, 다르면 변환해서 저장 
					
					// * 중요! value를 그냥 넣는게 아니라, 변환을 해야한다. 
